...
function Opening({ onFinish }: { onFinish: () => void }) {
  ...
  return <div className="opening" role="dialog" aria-label="Abertura do site" aria-modal="true"><div className="opening-tiles" aria-hidden="true">{Array.from({ length: 384 }, (_, index) => { const tileNumber = ((index * 11) % 60) + 1; return <span key={index} style={{ '--tile-index': index, backgroundImage: 'url(/tiles/tile-' + String(tileNumber).padStart(2, '0') + '.png)' } as React.CSSProperties} /> })}</div><div className="opening-content"><img className="opening-logo" src={siteContent.couple.logoSrc} alt="" /><p className="eyebrow">{siteContent.home.openingLabel}</p><p className="opening-date">{siteContent.couple.date}</p><h1>{siteContent.home.headline}</h1><p>{siteContent.couple.names}</p></div><button className="opening-skip" type="button" onClick={onFinish}>Pular abertura</button></div>
}
